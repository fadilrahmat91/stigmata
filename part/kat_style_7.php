...
<div class="div_height_kat col-sm-3" id="grid_produk">
			<?php
					
								
			$q_1 = $db->query("SELECT a.*, 
										 b.nama_kategori,							
										 d.nama_brand										
									FROM tbl_barang a
									...
									a.harga_coret > a.harga_barang AND a.harga_coret <>0 LIMIT 0,1
			...
			$hit_q= mysqli_num_rows($q);
					...
					echo '<div class="col-xs-12 col-sm-12 nopadding product_grid ">						
	...
	<div class="kat_gambarnya nopadding col-sm-6">
			...
			$qb = $db->query("SELECT a.id_barang, 
									...
									WHERE a.id_kategori='$id_kategori' ORDER BY rand() LIMIT 6");
			while($brand = $qb->fetch_object())
			{
				echo '<div class="col-xs-2" style="border-right:1px solid #ddd; background:red;padding:0px;margin:0px;">
	...
	<div class="div_height_kat col-sm-3" id="grid_produk">
			...
			$q_2 = $db->query("SELECT a.*, 
									...
									INNER JOIN tbl_brand d
										ON a.id_brand = d.id_brand
									WHERE a.id_kategori='$id_kategori' AND status_barang='1' AND
									a.harga_coret > a.harga_barang AND a.harga_coret <>0 LIMIT 1,1
