...
if(isset($_POST['page_edit_simpan']))
{
	$isi_sub_page	= $_POST['isi_sub_page'];
	$nama_sub_page	= $_POST['nama_sub_page'];
	$id_page	= $_POST['id_page'];
	$id_sub_page	= $_POST['id_sub_page'];
	
	//echo "INSERT INTO tbl_sub_page SET isi_sub_page='$isi_sub_page', nama_sub_page='$nama_sub_page', page_time='$page_time'";
	if($db->query("UPDATE tbl_sub_page SET id_page='$id_page', isi_sub_page='$isi_sub_page', nama_sub_page='$nama_sub_page',status='1' WHERE id_sub_page='$id_sub_page'"))
	{
...
$row  = $db->query("SELECT * FROM tbl_sub_page WHERE id_sub_page='$id_sub_page'")->fetch_object();
...
<div class="container">
	...
	<div id="alert_placeholder"></div>	
	  ...
	  <form class="form-horizontal" role="form" method="post" id="form_edit_sub_page" action="part/form_sub_page.php">
	  <input type="hidden" class="form-control" name="page_edit_simpan">
			...
			<select name="page_id" class="form-control" id="page_id">
				...
				<?php 
					...
					while($data = $q->fetch_object())
					{
						...
						$selected = $row->id_page==$data->page_id?'selected="selected"':'';
						echo '<option value="'.$data->page_id.'" '.$selected.'>'.$data->page_judul.'</option>';
					}
				?>
			</select>
		...
		<div id="toogle_main_form" >
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-10">
						<!--auto compelete-->
						<input type="text" class="form-control  nama_sub_page" id="nama_sub_page" name="nama_sub_page" value="<?php echo $row->nama_sub_page?>" required>			
					...
					<div class="form-group">
					  ...
					  <div class="col-sm-10">
						<!--auto editor-->
						<textarea class="form-control  isi_sub_page " id="isi_sub_page" name="isi_sub_page" value=""><?php echo $row->isi_sub_page?></textarea>
...
//submit form
$("#form_edit_sub_page").submit(function(){
	
	loading_menu();
	var nama_sub_page  	= $("#nama_sub_page").val();
	var id_sub_page  = '<?Php echo $_GET['id_sub_page']?>';
	var isi_sub_page  = $("#isi_sub_page").val();
	var 	id_page  = $("#page_id").val();
	 

	var page_edit_simpan ='page_edit_simpan';
	
	$.post("part/form_edit_sub_page.php",{id_sub_page:id_sub_page,	page_edit_simpan:page_edit_simpan,	id_page:id_page,nama_sub_page:nama_sub_page,isi_sub_page:isi_sub_page},function(e){
